...
public class ReplyDao extends DB {
	...
	public ArrayList<Reply> replies(int v_no) {
		...
		try {
			...
			while (resultSet.next()) {
				Reply reply = new Reply(resultSet.getInt(1), resultSet.getInt(2), resultSet.getInt(3),
						resultSet.getString(4), resultSet.getString(5));
				// System.out.println("reply : " + reply.getR_no() + ", " + reply.getM_no() + ",
				// " + reply.getV_no() + ", " + reply.getC_name());
				replies.add(reply);
	...
	// 댓글 번호
	public int replyno(int m_no, int v_no) {
		String sql = "select r_no from reply where m_no = ? and v_no = ?";
		try {
			preparedStatement = connection.prepareStatement(sql);
			preparedStatement.setInt(1, m_no);
			preparedStatement.setInt(2, v_no);
			resultSet = preparedStatement.executeQuery();
			if (resultSet.next()) {
				return resultSet.getInt(1);
			}
