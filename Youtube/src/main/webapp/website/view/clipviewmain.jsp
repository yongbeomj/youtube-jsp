                     ...
                     <div class="col-md-12">
                        <div class="row">
                           <!-- 영상 상세정보의 멤버 이미지(m_image), 구독자 수, 채널 명 -->
                              <!-- 로그인이 되어있고 회원 넘버가 메인 영상의 회원넘버랑 같을 때 채널 이동-->
                              <%
                              
                              if (login!=null && login.getM_no()==video.getM_no()) {%>
                                 <%
                                 int c_no = ChannelDao.getChannelDAO().getChannelNo(VideoDao.getVideoDAO().getM_no(v_no));
                                 System.out.println("c_no : " + c_no);
                                 f_checkcount = FollowDao.getFollowDao().followerCount(c_no);
                                 System.out.println("f_checkcount : " + f_checkcount);
                                 %>
                                 <div>
                                    <!-- 메인 영상의 멤버 이미지(m_image) -->
                                    <a href="../view/channel/newchannel.jsp"> <img src="../upload/<%=ReplyDao.getReplyDao().findm_image(video.getM_no())  %>" width="48px" height="48px" style="border-radius: 50%;"></a> 
                                 </div>   
                                 <div id="fallobox" class="col-md-8 pr-1 mr-3">
                                       <!-- 메인 영상 채널명 -->
                                    <a id="channelname" href="../view/channel/newchannel.jsp"><%=ChannelDao.getChannelDAO().getChannelName(video.getM_no()) %> </a>
                                    <br><span>팔로워 : <%=f_checkcount %> 명</span>
                                    <% f_checkcount = FollowDao.getFollowDao().followerCount(c_no); %>
                                 </div>   
                              <!-- 로그인이 되어있고 회원 넘버가 메인 영상의 회원넘버랑 같지 않을 때  채널 이동 -->
                              <%}else if (login!=null && login.getM_no()!=video.getM_no()){ 
                                 int c_no = ChannelDao.getChannelDAO().getChannelNo(VideoDao.getVideoDAO().getM_no(v_no));
                                 System.out.println("c_no : " + c_no);
                                 f_checkcount = FollowDao.getFollowDao().followerCount(c_no);
                                 System.out.println("f_checkcount : " + f_checkcount);
                              %>
                                 ...
                                 <div>
                                    <a href="../view/channel/otherchannel.jsp?c_no=<%=ChannelDao.getChannelDAO().getChannelNo(VideoDao.getVideoDAO().getM_no(v_no))%>"> <img src="../upload/<%=ReplyDao.getReplyDao().findm_image(VideoDao.getVideoDAO().getM_no(v_no)) %>" width="48px" height="48px" style="border-radius: 50%;"></a>
                                 </div>
                                 <div id="fallobox" class="col-md-8 pr-1 mr-3">
                                    <!-- 메인 영상 채널명 -->
                                    <a id="channelname" href="../view/channel/otherchannel.jsp?c_no=<%=ChannelDao.getChannelDAO().getChannelNo(VideoDao.getVideoDAO().getM_no(v_no))%>"><%=ChannelDao.getChannelDAO().getChannelName(VideoDao.getVideoDAO().getM_no(v_no)) %> </a>
                                    <br><span>팔로워 : <%=f_checkcount %> 명</span>
                                 </div>
                           ...
                           <!-------------------메인 영상 끝 ------------------------->   
                           <!------------------- 팔로우 시작 --------------------------->
                           <div class="col-md-3 pl-3 pr-0 row d-flex justify-content-end">
                        
                              <%
                              //System.out.println("로그인세션: " + login);
                              //System.out.println("로그인세션2: " + login);
                              if(login != null && login.getM_no()!=VideoDao.getVideoDAO().getM_no(v_no)){ //로그인 아이디가 존재하면 로그인 m_no가 
                                 //System.out.println("로그인세션2: " + login);
                                 int m_no = MemberDao.getMemberDao().getmemberno(member.getM_id());
                                 System.out.println("mno : " + m_no);
                                 int c_no = ChannelDao.getChannelDAO().getChannelNo(VideoDao.getVideoDAO().getM_no(v_no));
                                 System.out.println("cno : " + c_no);
                                 boolean f_check = FollowDao.getFollowDao().followcheck(c_no, m_no);
                                 System.out.println("f_check : " + f_check);
                                 f_checkcount = FollowDao.getFollowDao().followerCount(c_no);
                                 ...
                                 //팔로우중이라면
                                 if(f_check  == true){%>
                                    <a href="#" id ="followbtn" class="md-4" onclick="c_follow2(<%=c_no%>,<%=m_no%>);">
                                 ...
                                 %>
                                    ...
                                    <img id="videolike" class="ml-3 mt-2" src="../img/heartfill.png" onclick="v_like(<%=v_no%>,<%=m_no%>);"
                                    style="height: 30px; width: 30px; cursor: pointer;">
                                 <%
                                 } else { //좋아요가 존재하지 않으면
                                 %>
                                    <img id="videolike" class="ml-3 mt-2" src="../img/heart.png" onclick="v_like(<%=v_no%>,<%=m_no%>);"
                                    style="height: 30px; width: 30px; cursor: pointer;">
                                 <%
                                 }
                              }
                     ...
                     <div class="collpase navbar-collapse">
                        <div class="navbar-nav">
                           ...
                           <details class="nav-item">
                              <summary id="clipinfo">영상설명 펼치기</summary>
                        ...
                        <!-- 댓글 창에서 로그인이 되어있고 회원 넘버가 영상의 회원넘버랑 같을 때 채널 이동-->
                           <%if (login!=null && login.getM_no()==replies2.get(j).getM_no()) {%>
                              ...
                              <div class="col-md-9 pl-2 mr-0">
                                 <a id="channelname" href="../view/channel/newchannel.jsp"><%=ChannelDao.getChannelDAO().getChannelName(replies2.get(j).getM_no())%></a>
                           ...
                           <%}else if(login!=null && login.getM_no()!=replies2.get(j).getM_no()){%>
                              ...
                              <!-- 댓글창의 댓글 내용 (채널명, 댓글 내용) -->
                              <div class="col-md-9 pl-2 mr-0">
                                 <a id="channelname" href="../view/channel/otherchannel.jsp?c_no=<%=ChannelDao.getChannelDAO().getChannelNo(replies2.get(j).getM_no())%>"><%=ChannelDao.getChannelDAO().getChannelName(replies2.get(j).getM_no())%></a>
                                 <div class="pl-0 my-2"><%=replies2.get(j).getR_contents()%></div>
                              </div>
                           <!-- 비회원 채널 이동 -->   
                           <%}else{%>
                     ...
                     <div class="row col-md-12 m-0 ">
                        ...
                        <div class="col-md-2 pl-0 mb-4" style="border-radius: 15px;">
                           ...
                           <div class="col-md-12 mt-2 pl-0 pr-0">
                              <!-- 추천 영상 제목 -->
                              <div ><a href="clipviewmain.jsp?v_no=<%=videolist.get(i).getV_no()%>" id="channelrname" ><%=videolist.get(i).getV_title()%></a></div>
                              
                              <!-- 로그인이 되어있고 회원 넘버가 영상의 회원넘버랑 같을 때 채널 이동-->
                              <%if (login!=null && login.getM_no()==videolist.get(i).getM_no()) {%>
                                 <!-- 추천 영상 채널 이미지(m_image) -->
                                 <a  href="../view/channel/newchannel.jsp"> 
                                    <img src="../upload/<%=ReplyDao.getReplyDao().findm_image(VideoDao.getVideoDAO().getM_no(videolist.get(i).getV_no())) %>" width="20" height="20" style="border-radius: 50%;">
                                    
                                 </a> 
                                 <!-- 추천 영상 채널명 -->
                                 <a href="../view/channel/newchannel.jsp"> 
                                    <label id="channelname"><%=ChannelDao.getChannelDAO().getChannelName(VideoDao.getVideoDAO().getM_no(videolist.get(i).getV_no())) %></label>
                                 </a>
                              <!-- 로그인이 되어있고 회원 넘버가 영상의 회원넘버랑 같지 않을 때 채널 이동 -->
                              <%}else if (login!=null && login.getM_no()!=videolist.get(i).getM_no()){ %>
                                 <!-- 추천 영상 채널 이미지(m_image) -->   
                                 <a  href="../view/channel/otherchannel.jsp?c_no=<%=ChannelDao.getChannelDAO().getChannelNo(VideoDao.getVideoDAO().getM_no(videolist.get(i).getV_no()))%>"> 
                                    <img src="../upload/<%=ReplyDao.getReplyDao().findm_image(VideoDao.getVideoDAO().getM_no(videolist.get(i).getV_no())) %>" width="20" height="20" style="border-radius: 50%;">
                                 </a> 
                                 <!-- 추천 영상 채널명 -->
                                 <a href="../view/channel/otherchannel.jsp?c_no=<%=ChannelDao.getChannelDAO().getChannelNo(VideoDao.getVideoDAO().getM_no(videolist.get(i).getV_no()))%>"> 
