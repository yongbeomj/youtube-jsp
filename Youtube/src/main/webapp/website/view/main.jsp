...
<body>
   <%
      ...
      if(search != null) {
         ArrayList<Video> videos2 = VideoDao.getVideoDAO().Search(search);
   %>
            ...
            <div class = "row justify-content-between ml-3">
               <%
                  for(int i = 0; i < videos2.size() ; i++){
                     int v_no = videos2.get(i).getV_no();
                     int m_no = VideoDao.getVideoDAO().getM_no(v_no);
                     int channelno = ChannelDao.getChannelDAO().getChannelNo(m_no);
                     System.out.print(m_no);
                     Member member2 =  MemberDao.getMemberDao().mnoselect(videos.get(i).getM_no());
                     //String m_image = MemberDao.getMemberDao().getImage(m_no);
                     Channel channel = ChannelDao.getChannelDAO().getSoloChannel(channelno);
                     System.out.println(videos2.get(i).getV_thumbnail().split("_")[0]);
               %>
                     <div class = "card col-xs-8 offset-xs-2 m-2" style="width: 16rem;">
                        <a href = "clipviewmain.jsp?v_no=<%=v_no%>">
                           <img alt="" src="../upload/<%= videos2.get(i).getV_thumbnail().split("_")[0] %>" class = "card-img-top" style="max-width: 256px; max-height: 144px;">
                        </a>
                        <%-- <a href = "희동이페이지"><video style="width: 100%; height: 100%;" src = "../upload/<%= videos2.get(i).getV_name().split("_")[0] %>" autoplay="autoplay" muted="muted" poster="<%= videos2.get(i).getV_thumbnail().split("_")[0] %>"></video></a> --%>
                        <div class = "card-body row" style="height: 6rem;">
                           <div class = "col-md-2 pt-2 col-xs-2 col-sm-2" style=" padding-left: 0; margin-left: 0">
                              <a href = "channel/otherchannel.jsp?c_no=<%= channel.getC_no() %>">
                                 <img alt="" src="../upload/<%= member2.getM_image() %>" style="width: 2.5rem; height: 2.5rem; border-radius: 70%;">
                              </a>
                           </div>
                           <div class = "col-md-10 col-xs-10 col-sm-10">
                              <%
                                 if(videos2.get(i).getV_title().split(".mp4")[0].length() > 16) {
                                    String videoTitle = videos2.get(i).getV_title().split(".mp4")[0].substring(0,16) + "...";
                              %>
                                    <h5 class = "card-title" style="font-size: 16px;"><a href = "#" style="text-decoration: none; color: black;"><%= videoTitle %></a></h5>
                              <%
                                 } else {
                              %>
                                    <h5 class = "card-title" style="font-size: 16px;"><%= videos2.get(i).getV_title().split(".mp4")[0] %></h5>
                              <%
                                 }
                              %>
            ...
            <div class = "row justify-content-between ml-3">
               <%
                  ...
                  for(int i = 0; i < videos.size() ; i++){
                     ...
                     Member member2 =  MemberDao.getMemberDao().mnoselect(videos.get(i).getM_no());
                     Channel channel = ChannelDao.getChannelDAO().getSoloChannel(channelno);
                     System.out.println(videos.get(i).getV_thumbnail().split("_")[0]);
               %>
                     <div class = "card col-xs-8 offset-xs-2 m-2" style="width: 16rem;">
                        <a href = "clipviewmain.jsp?v_no=<%=v_no%>">
